...
class CallbackHandler(webapp2.RequestHandler):
	def get(self,):
		...
		result = json.loads(urlfetch.fetch(url='https://www.googleapis.com/oauth2/v4/token', payload = json.dumps(payload), method=urlfetch.POST))
		
		auth_key = "Bearer " + result.access_token
		
		payload = {'Authorization': auth_key}
		response = json.loads(urlfetch.fetch(url='https://www.googleapis.com/plus/v1/people/me', payload = json.dumps(payload), method=urlfetch.GET))
